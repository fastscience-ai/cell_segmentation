...
class ConvLSTM_Seg(nn.Module):
    ...
    def forward(self, X, H, C): 
        ...
        output, h1, c1 = self.layer1(X, H[0], C[0])
        output, h2, c2 = self.layer2(output, H[0], C[0])
        output, h3, c3 = self.layer3(output, H[0], C[0])
        output, h4, c4 = self.layer4(output, H[0], C[0])
...
def main():
    ...
    optim = torch.optim.Adam(convlstm.parameters(), lr=0.000001) #working lr for mse 0.0001
    criterion = nn.BCELoss()#reduction="sum") #BCELoss()   
    ...
    trouble_shooting_out , h, c = convlstm(X.float().cuda(), 
                                           torch.from_numpy(np.asarray([False for i in range(4)])).cuda(), torch.from_numpy(np.asarray([False for i in range(4)])).cuda())
    ...
    for epoch in range(EPOCHS):
        ...
        for jj in range(9):
            ...
            #print(np.shape(train_x), np.shape(train_y), np.amax(train_x), np.amin(train_x), np.amax(train_y), np.amin(train_y))
            mm = len(train_y)
            train_x = torch.permute(torch.from_numpy(train_x),(0,4,1,2,3))
            train_y = torch.permute(torch.from_numpy(train_y),(0,4,1,2,3))
            #print(train_x.size(), train_y.size()) #torch.Size([20, 2, 15, 70, 70]) torch.Size([20, 3, 15, 70, 70])
            
            for k in range(len(train_x)):
            
                output,  h, c = convlstm(train_x[k:k+1].float().cuda(),  
                                     torch.from_numpy(np.asarray([False for i in range(4)])).cuda(), torch.from_numpy(np.asarray([False for i in range(4)])).cuda())
                loss = criterion(output.flatten().float().cuda(), train_y[k:k+1].flatten().float().cuda())   
                loss.backward()
                optim.step()
                optim.zero_grad()
        ...
        with torch.no_grad():
            ...
            for k in range(len(test_x)):
                output, h, c = convlstm(test_x[k:k+1].float().cuda(), 
                                        torch.from_numpy(np.asarray([False for i in range(4)])).cuda(), 
                                        torch.from_numpy(np.asarray([False for i in range(4)])).cuda())
                out_.append(output.cpu().numpy())                        
                loss = criterion(output.flatten().float().cuda(), test_y[k:k+1].flatten().float().cuda())
                #print(loss.item(), np.shape(output.numpy()))
                val_loss += loss.item()  
        output = np.concatenate(out_,0)                
        val_loss = val_loss/float(len(test_x))
        print("Epoch:{} Training Loss:{:.2f} Validation Loss:{:.2f}\n".format(
        epoch, train_loss, val_loss))
        hist_all.append([epoch, train_loss, val_loss])
        if epoch % 10 == 0:
            torch.save(convlstm.state_dict(), "./convlstm_torch_epoch_"+str(epoch)+"_bce_"+str(val_loss))
