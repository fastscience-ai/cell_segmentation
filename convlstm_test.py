...
def masking(folder='./data/Bcell_1/'):
    ...
    for sample in cells:
        ...
        labels_new =np.zeros([d1,d2,d3,2])
...
def downscale( image, label, mag):
    ...
    for i in range(d1):
        ...
        y = np.resize(y, [1, int(d2/mag),int(d3/mag), 2])
        image_.append(x)
        label_.append(y)
...
def crop_video(collect_model, stride, size, timestep, file_list,jj):
    ...
    for image, label in list(zip(x_t[:], y_t[:])):
        ...
        for t in range(len(image)-timestep):
            input_file.append(np.reshape(image[t:t+timestep], [1,timestep,int(size/4),int(size/4),1]))
            output_file.append(np.reshape(label[t:t+timestep],[1,timestep,int(size/4),int(size/4),2]))
    input_file_ =[]
    output_file_ = []
...
def main():
    convlstm = tf.keras.models.load_model('convlstm_latest')
    # Check its architecture

    print(convlstm.summary())
    #test
    test_x, test_y = crop_video(False, 35, 70, 10, ["Bcell_test"], 0)
    print(np.shape(test_x), np.shape(test_y))
    #Evaluate with test set
    for i in range( len(test_x)):
        test_predictions = convlstm.predict(test_x[i:i+1])
        test_predictions = test_predictions[:,:,:,:,:]
        iou, dice_score = compute_metrics(test_y[i:i+1][0, -1], test_predictions[0, -1], 2)
        print(i, iou, dice_score, np.shape(test_x[i:i+1]), np.shape(test_y[i:i+1]), np.shape(test_predictions))
