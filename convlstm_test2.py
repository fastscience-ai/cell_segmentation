...
def downscale( image, label, mag):
    ...
    for i in range(d1):
        ...
        y = np.resize(y, [1, int(d2/mag),int(d3/mag), 6])
        image_.append(x)
        label_.append(y)
...
def crop_video(collect_model, stride, size, timestep, file_list,jj):
    ...
    for image, label in list(zip(x_t[:], y_t[:])):
        ...
        for t in range(len(image)-timestep):
            ...
            lab = np.reshape(label[t:t+timestep],[1,timestep,int(size/4),int(size/4),6])
            image_concat=[]
            for k in range(5):
...
def main():
    convlstm = tf.keras.models.load_model('convlstm2_latest')#'convlstm2_latest_good')
