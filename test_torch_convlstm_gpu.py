...
def main():
    ...
    convlstm.load_state_dict(torch.load("./convlstm_torch_epoch_90_mse_0.058645381682342096"))
    convlstm.eval()
    ...
    with torch.no_grad():
        ...
        for i in range(len(test_x)):
            if i == 0:
                out, h, c = convlstm(test_x[i:i+1].float().cuda(), 
                ...
                for j in range(len(h)):
                    h_.append(np.float32(h[j].cpu()))
                    c_.append(np.float32(c[j].cpu()))
                h_ =np.asarray(h_)
                c_ = np.asarray(c_)
                print("type", type(h_), np.shape(h_))
                out, h, c = convlstm(test_x[i:i+1].float().cuda(), 
                                                  torch.from_numpy(h_).cuda(), 
                                                  torch.from_numpy(c_).cuda())                                                        
        ...
        print(np.shape(test_predictions), np.shape(test_y))
        for i in range(len(test_x)):
